...
test.describe("Local Recording Export Workflow", () => {
	test("loads local recording and exports MP4 with annotation", async () => {
		...
		try {
			...
			// Verify video state with retries
			let videoState: { exists: boolean; readyState?: number; duration?: number } | null = null;
			let retries = 3;
			while (retries > 0) {
				videoState = await editorWindow.evaluate(() => {
			...
			console.log("[TEST] Step 8: Selecting MP4 format...");
			...
			console.log("[TEST] Step 11: Retrieving exported data...");
